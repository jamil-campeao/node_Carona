...
export const getCalculo = async (req, res) => {
  ...
  try {
    ...
    caronas.forEach((carona) => {
      const chaveViagem = `${carona.CAR_DATA.toISOString()}-${
        carona.destino.DES_ID
      }`;
    ...
    for (const chaveViagem in viagensAgrupadas) {
      ...
      const custoTotalDaViagem =
        (viagem.distancia / mediaCarro) * precoGasolina;
    ...
    contagemViagensPassageiro[nomeMotorista] =
      Object.keys(viagensAgrupadas).length;

    // 4. Formatar o resultado final
    const detalhesPassageiros = {};
    const todosOsNomes = new Set(
      Object.keys(contagemViagensPassageiro).concat(
        Object.keys(valoresIndividuais)
      )
    );
    ...
    todosOsNomes.forEach((pessoa) => {
      detalhesPassageiros[pessoa] = {
        ...
        valorTotal: parseFloat(
          (valoresIndividuais[pessoa]?.valorTotal || 0).toFixed(2)
        ),
      };
    });
    ...
    res
      .status(500)
      .json({ error: `Erro ao calcular valores: ${error.message || error}` });
  }
};
...
export const getQuantidadesViagensDia = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Erro ao executar requisição: ${error.message}` });
  }
};
